...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  // İlk kenarı bekle
	  //timera bağlı pin üzerinde sinyal kenarı gelene kadar bekle
	  while (__HAL_TIM_GET_FLAG(&htim2, TIM_FLAG_CC1) == RESET);
	  //set olana kadar
	  __HAL_TIM_CLEAR_FLAG(&htim2, TIM_FLAG_CC1);
	  //bayrağı temizle tekrar beklemeye hazır ol
	  capture1 = HAL_TIM_ReadCapturedValue(&htim2, TIM_CHANNEL_1);
//o anda tuttuğu değeri oku değişkene ata
	  // İkinci kenarı bekle
	  while (__HAL_TIM_GET_FLAG(&htim2, TIM_FLAG_CC1) == RESET);
	  __HAL_TIM_CLEAR_FLAG(&htim2, TIM_FLAG_CC1);
